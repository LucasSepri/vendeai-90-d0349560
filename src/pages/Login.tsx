...
const Login = () => {
  ...
  return <div className="flex min-h-screen w-full bg-black relative overflow-hidden">
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 lg:w-[500px] p-6 z-10">
        <div className="backdrop-blur-xl bg-black/40 border border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.3)] rounded-xl p-10 w-full max-w-[440px] px-[40px] py-[34px]">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <Input type="email" placeholder="Seu E-mail" value={email} onChange={e => setEmail(e.target.value)} className="bg-transparent border border-white/20 text-white h-12 w-full px-4 rounded-lg 
                          focus:border-[#a66717] focus:ring-1 focus:ring-[#a66717] transition-all duration-300
                          placeholder:text-white/60" required />
            </div>
            
            <div className="relative">
              <Input type={showPassword ? "text" : "password"} placeholder="Sua Senha" value={password} onChange={e => setPassword(e.target.value)} className="bg-transparent border border-white/20 text-white h-12 w-full px-4 rounded-lg 
                          focus:border-[#a66717] focus:ring-1 focus:ring-[#a66717] transition-all duration-300
                          placeholder:text-white/60" required />
              <button type="button" onClick={togglePasswordVisibility} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-white/60 hover:text-white">
                {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
              </button>
            </div>
            
            <Button type="submit" className="w-full h-12 bg-white hover:bg-[#a66717] hover:text-white text-black flex items-center justify-center font-medium text-base transition-colors">
              Acessar sua conta 
      ...
      <div className="hidden md:flex flex-col justify-center items-start w-1/2 p-12 z-10">
        <h2 className="text-7xl font-bold text-white leading-tight px-[89px] py-[11px]">
